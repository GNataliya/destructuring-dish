const test = () => {
  ...
  let { status, ingridients } = test();
  
  let isAlergen = ingridients.filter((ingridient) => {
      return ingridient.alergen === false;
  });

  let KkalSumm = isAlergen.reduce((sum, ingridients) => {
    return sum + ingridients.kkal;
  }, 0);

  console.log(KkalSumm)
